...
class BookStore:
  ...
  def load_catalog(self, fileName: str, ds: str, sort_by: str):
    ...
    # sorting catalog
      if sort_by == '1':
        algorithms._quick_sort_r(self.bookCatalog, 0,
                                 self.bookCatalog.size() - 1)
        ...
        # FIXME: Update self.catalogIsSorted
        algorithms._quick_sort_f(self.bookCatalog, 0,
                                 self.bookCatalog.size() - 1)
        self.catalogIsSorted = True
        ...
        print(
            "Invalid sorting algorithm selected.  Catalog will not be sorted.")
      ...
      print(
          f"Loaded {self.bookCatalog.size()} books in {elapsed_time} seconds.")
  ...
  def get_book_at_index(self, i: int):
    ...
    try:
      ...
      elapsed_time = time.time() - start_time
      print(
          f"Accessed the following book from catalog:{book}\nAction completed in {elapsed_time} seconds."
      )
    except IndexError:
      print(
    ...
    if not by_rank:
      ...
      try:
        ...
        for i in range(self.bookCatalog.size()):
          book = self.bookCatalog.get(i)
          if infix.lower() in book.title.lower():
            b = Book(book.key, book.title, book.group, -1 * book.rank,
                     book.group)
  ...
  def search_by_title(self, title: str, search_algo):
    ...
    try:
      if search_algo == '1':
        ...
        idx = algorithms.linear_search(
            self.bookCatalog, title)  # FIXME: Replace with the correct code
  ...
  def search_by_prefix(self, prefix, n):
    ...
    try:
      ...
      if r is None:
        ...
        nodes = bt.bf_order()  #FIXME: Replace with the correct code
