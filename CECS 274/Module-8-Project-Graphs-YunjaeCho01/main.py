    ...
    while option.upper() != 'Q':
        ...
        if option == '1':
            ...
            if calculator.balanced_parens(expression):
                expr_with_values, vars_without_vals = calculator.expr_with_values(
                    expression)
                print("Expression with values:", expr_with_values)
                if len(vars_without_vals) > 0:
                    print(
                        "Can NOT evaluate expression because the following variables have not been assigned values:"
                    )
