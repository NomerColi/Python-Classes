...
class BookStore:
    ...
    def load_catalog(self, fileName: str, catalog_ds: str, graph_ds: str,
                     sort_by: str):
        """
        ...
        try:
            if catalog_ds == '1':
                ...
                raise ValueError(
                    f"'{catalog_ds}' is not a valid catalog data structure option."
                )
            ...
            for i in range(self.bookCatalog.size()):
                ...
                self.mapKeysToIdxs.add(
                    book.key,
                    i)  # chained-hash table maps (book key, catalog idx)
                self.mapTitlesToIdxs.add(
                    book.title.lower(),
                    i)  # binary search tree maps (book title, catalog idx)

            n = self.bookCatalog.size()

            # loading graph
            if graph_ds == '1':
                self.similarityGraph = AdjacencyList(n)
            elif graph_ds == '2':
                self.similarityGraph = AdjacencyMatrix(n)
            else:
                raise ValueError(
                    f"'{graph_ds}' is not a valid graph data structure option."
                )
            ...
            for source_node in range(self.bookCatalog.size()):
                ...
                similar = book.similar.split(
                    " ")  # Splitting the string of keys to similar books
                ...
                for j in range(len(similar)):
                    ...
                    target_node = self.mapKeysToIdxs.find(
                        key)  # FIXME: Get the index corresponding to the key
            ...
            print(
                "\nLoad Summary:\n" + msg +
                f"\n- Loaded and sorted {n} items.\n- Created graph with {n} nodes and {m} edges."
    ...
    def get_book_at_index(self, i: int):
        ...
        try:
            ...
            print(
                f"Index {i} is out of bounds for catalog of size {self.bookCatalog.size()}"
            )
        except Exception as e:
            print("The following unexpected error occurred:")
            traceback.print_exc(limit=2, file=sys.stdout)
        ...
        if not by_rank:
            ...
            try:
                ...
                for i in range(self.bookCatalog.size()):
                    book = self.bookCatalog.get(i)
                    if infix.lower() in book.title.lower():
                        b = Book(book.key, book.title, book.group,
                                 -1 * book.rank, book.group)
            ...
            if search_algo == '1':
                ...
                if self.catalogIsSorted:
                    ...
                    raise InvalidStateException(
                        "Cannot perform Binary Search on unsorted catalog.")
            else:
                raise ValueError(
                    "Invalid searching algorithm was selected.  Please try again."
                )
            if idx is None:
                print(
    ...
    def remove_from_shopping_cart(self):
        ...
        try:
            ...
            if self.shoppingCart.size() > 0:
                ...
                elapsed_time = time.time() - start_time
                print(
                    f"Nothing to remove.  Shopping cart is empty.\nAction completed in {elapsed_time} seconds."
                )
        except Exception as e:
            print("The following unexpected error occurred:")
            traceback.print_exc(limit=2, file=sys.stdout)
    ...
    def _display_similar(self, idx: int):
        ...
        try:
            similar = self.dtype(
            )  # Initializing 'similar' to be an empty ArrayList/DLList

            for edge in self.similarityGraph.out_edges(idx):
                similar.append(edge)
            ...
            if similar.size() > 0:
                ...
                for i in range(similar.size()):
                    ...
                    print("%-15s | %-15s | %-15s | %-100s |" %
                          (similar_book.key, similar[i], similar_book.group,
                           similar_book.title))
        except IndexError:
            print(
                f"Index {idx} is out of bounds for catalog of size {self.bookCatalog.size()}"
            )
        except Exception as e:
            print("The following unexpected error occurred:")
            traceback.print_exc(limit=2, file=sys.stdout)
