...
class BookStore:
  ...
  def loadCatalog(self, fileName: str):
    ...
    try:
      ...
      print(
          "Encountered the following unexpected error while loading the catalog:\n",
          e)
  ...
  def addToCatalog(self, i, book: Book.Book):
    ...
    try:
      ...
      elapsed_time = time.time() - start_time
      print(
          f"Inserted the following book at index {i}:{book}\nAction completed in {elapsed_time} seconds."
      )
    except IndexError:
      print(
  ...
  def removeFromCatalog(self, i: int):
    ...
    try:
      ...
      elapsed_time = time.time() - start_time
      print(
          f"Removed the following book from catalog:{removed_book}\nCatalog size after removal: "
  ...
  def searchBookByInfix(self, infix: str, k: int):
    ...
    try:
      ...
      for i in range(n):
        ...
        if b.title.lower().find(infix.lower()) != -1:
          print("-" * 25 + f"\nMatch found at catalog index {i}:\n{b}\n")
          printed += 1
          if printed == k:
            break
  ...
  def addBookByIndex(self, i: int):
    ...
    try:
      ...
      # FIXME: Correct the line below
      s = self.bookCatalog.get(
          i
      )  #"""FIXME: Replace this string with the code that will assign to s the book at index i of the catalog"""
